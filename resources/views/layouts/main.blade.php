...
<head>
    ...
    @auth
        <meta name="user-profile-id" content="{{ auth()->user()->id }}">
        <meta name="user-level-name" content="{{ auth()->user()->hasRole('admin') ? 'Admin' : (auth()->user()->hasRole('instructor') ? 'Instructor' : 'User') }}">
    @endauth

    <title>{{ config('app.name', 'MaxCourse') }} - @yield('title', 'Platform Pembelajaran Online')</title>
                        ...
                        <div class="relative">
                            <button id="user-menu-button" class="flex items-center space-x-2 text-gray-700 hover:text-gray-900 focus:outline-none">
                                @if(auth()->user()->avatar_url)
                                    <img src="/storage/{{ auth()->user()->avatar_url }}" alt="Profile Picture" class="w-8 h-8 rounded-full object-cover">
                                @else
                            ...
                            <div id="user-menu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-1 z-50">
                                @hasrole('admin')
                                    {{-- Profile link tidak ditampilkan untuk admin --}}
                                @else
    ...
    <footer class="bg-gray-900 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                @hasrole('admin')
                    {{-- Admin: tidak tampilkan tautan cepat --}}
                @else
                <div>
                ...
                @hasrole('admin')
                    {{-- Admin: tidak tampilkan dukungan --}}
                @else
                <div>
            ...
            <div class="border-t border-gray-800 mt-8 pt-8 text-center">
                <p class="text-gray-400">
                    &copy; {{ date('Y') }} MaxCourse. Hak cipta dilindungi undang-undang.
                </p>
            </div>
        </div>
    </footer>
